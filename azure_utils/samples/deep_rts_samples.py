...
class ResNet152(RTSEstimator):
    ...
    def _identity_block(
        ...
        stacked_layers = BatchNormalization(
            epsilon=eps, axis=bn_axis, name=bn_name_base + "2a"
        )(stacked_layers)
        stacked_layers = Scale(axis=bn_axis, name=scale_name_base + "2a")(
            stacked_layers
        )
        stacked_layers = Activation("relu", name=conv_name_base + "2a_relu")(
            stacked_layers
        )

        stacked_layers = ZeroPadding2D((1, 1), name=conv_name_base + "2b_zeropadding")(
            stacked_layers
        )
        stacked_layers = Conv2D(
            nb_filter2,
        ...
        stacked_layers = BatchNormalization(
            epsilon=eps, axis=bn_axis, name=bn_name_base + "2b"
        )(stacked_layers)
        stacked_layers = Scale(axis=bn_axis, name=scale_name_base + "2b")(
            stacked_layers
        )
        stacked_layers = Activation("relu", name=conv_name_base + "2b_relu")(
            stacked_layers
        )

        stacked_layers = Conv2D(
            nb_filter3, (1, 1), name=conv_name_base + "2c", use_bias=False
        )(stacked_layers)
        stacked_layers = BatchNormalization(
            epsilon=eps, axis=bn_axis, name=bn_name_base + "2c"
        )(stacked_layers)
        stacked_layers = Scale(axis=bn_axis, name=scale_name_base + "2c")(
            stacked_layers
        )

        stacked_layers = add(
            [stacked_layers, input_tensor], name="res" + str(stage) + block
        )
        stacked_layers = Activation("relu", name="res" + str(stage) + block + "_relu")(
            stacked_layers
        )
